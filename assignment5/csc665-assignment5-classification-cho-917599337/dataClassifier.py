...
def enhancedFeatureExtractorDigit(datum):
    ...
    # iterate through x and y of DIGIT_DATUM_WIDTH and HEIGHT, if datum pixels for the x and y 
    # are in bound, update the features of horizontal or vertical to 1 (true) else 0 (false)
    for x in range(1, DIGIT_DATUM_WIDTH):
        for y in range(1, DIGIT_DATUM_HEIGHT):
            ...
            """print("features[(horiz, x, y)]: ", features[("horiz", x, y)] )"""
    ...
    for x in range(1, DIGIT_DATUM_WIDTH):
        for y in range(1, DIGIT_DATUM_HEIGHT):
            ...
            if y < DIGIT_DATUM_HEIGHT - 1:
                neighbors.append((x, y + 1))
            features["neighbors", x, y] = int(len(neighbors) > 2)
            features["noneighbors", x, y] = int(len(neighbors) == 0)
    return features
...
def enhancedPacmanFeatures(state, action):
    ...
    successor = state.generateSuccessor(0, action) #get successor of pacman with given action
    """print ("successor: ", type(successor))"""
    foodList = successor.getFood().asList() # get foodList from successor state
    for food in foodList: #iterate through the foooList to update features Array
        # use str(food) to use as feature name, use manhattanDistance to get featureValue
