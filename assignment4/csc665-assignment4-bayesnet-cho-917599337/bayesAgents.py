...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            obsFactor = bn.Factor([obsVar], HOUSE_VARS, variableDomainsDict)
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                currentPos = ""
                if housePos == bottomLeftPos:
                    currentPos = BOTTOM_LEFT_VAL
                elif housePos == topLeftPos:
                    currentPos = TOP_LEFT_VAL
                elif housePos == bottomRightPos:
                    currentPos = BOTTOM_RIGHT_VAL
                else:
                    currentPos = TOP_RIGHT_VAL

                food = assignment[FOOD_HOUSE_VAR] == currentPos
                ghost = assignment[GHOST_HOUSE_VAR] == currentPos
                ...
                if food:
                    ...
                    if assignment[obsVar] == NO_OBS_VAL: # when assignment[obsVar] == NO_OBS_VAL:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for assignment_Dict in factor.getAllPossibleAssignmentDicts():
        ...
        if probability > highest_Prob:
            ...
            highest_Assign_Dict = assignment_Dict
    return highest_Assign_Dict
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        left_Factor = evidence.copy()
        left_Factor[FOOD_HOUSE_VAR] = TOP_LEFT_VAL
        left_Factor[GHOST_HOUSE_VAR] = TOP_RIGHT_VAL

        right_Factor = evidence.copy()
        right_Factor[FOOD_HOUSE_VAR] = TOP_RIGHT_VAL
        right_Factor[GHOST_HOUSE_VAR] = TOP_LEFT_VAL
        
        leftExpectedValue = query.getProbability(left_Factor)*WON_GAME_REWARD + query.getProbability(right_Factor)*GHOST_COLLISION_REWARD
        rightExpectedValue = query.getProbability(right_Factor)*WON_GAME_REWARD + query.getProbability(left_Factor)*GHOST_COLLISION_REWARD
